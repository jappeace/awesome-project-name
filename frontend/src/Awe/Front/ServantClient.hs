...
getCookies =
  currentDocument >>= maybe (pure Nothing) (fmap Just . Doc.getCookie)
...
apiClients = clientWithOpts serviceAPI
                            (Proxy @m)
                            (Proxy @())
                            (constDyn url)
                            clientOpts
...
postMessage dd yy =
  fmap switchDyn $ prerender (pure never) $ postMessage' dd yy
...
getMe' :: MonadWidget t m => Event t () -> m (Event t (ReqResult () User))
postMessage'
  :: MonadWidget t m
  => Dynamic t (Either Text.Text Message)
  -> Event t ()
